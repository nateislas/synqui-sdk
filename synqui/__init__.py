"""Synqui Python SDK for observability and tracing.

This SDK provides easy instrumentation for Python applications to capture
trace data and send it to the Synqui platform.

Basic Usage (Recommended):
    import synqui

    # Simple initialization with sensible defaults
    synqui.init(api_key="your-api-key")
...
Manual Tracing:
    ...
    with synqui.span("custom_operation") as span:
        span.set_attribute("key", "value")
        # Your code here
"""

__version__ = "0.1.0"
__author__ = "Synqui Team"
__email__ = "team@synqui.com"
...
try:
    from .langchain import SynquiCallbackHandler, get_synqui_handler, capture_error
    _LANGCHAIN_AVAILABLE = True
except ImportError:
    _LANGCHAIN_AVAILABLE = False

try:
    from .langgraph import SynquiLangGraphHandler, get_synqui_langgraph_handler, create_langgraph_config
    _LANGGRAPH_AVAILABLE = True
except ImportError:
    _LANGGRAPH_AVAILABLE = False

# Default SDK instance
_default_sdk: Optional[SynquiSDK] = None


def get_default_sdk() -> SynquiSDK:
    """Get the default SDK instance."""
    global _default_sdk
    if _default_sdk is None:
        raise RuntimeError("SDK not configured. Call synqui.configure() first.")
    return _default_sdk


def set_default_sdk(sdk: SynquiSDK):
...
def start_chat_session(
    ...
    Example:
        ...
        handler = synqui.get_synqui_langgraph_handler_with_session(session)
    """
...
__all__ = [
    ...
    "start_chat_session",
    "get_session_manager",
    "get_synqui_langgraph_handler_with_session",
    "json_loads",
    "__version__",
]
...
if _LANGCHAIN_AVAILABLE:
    __all__.extend(["SynquiCallbackHandler", "get_synqui_handler", "capture_error"])

# Add LangGraph integration if available
if _LANGGRAPH_AVAILABLE:
    __all__.extend(["SynquiLangGraphHandler", "get_synqui_langgraph_handler", "create_langgraph_config"])


# Add session-aware handler function
def get_synqui_langgraph_handler_with_session(session: ChatSession):
    """Get a SynquiLangGraphHandler configured for a specific chat session.

    Args:
        session: The ChatSession instance to associate with the handler

    Returns:
        SynquiLangGraphHandler configured for the session

    Example:
        import synqui

        # Create session
        session = synqui.start_chat_session("my_chat")
    ...
    from .langgraph import SynquiLangGraphHandler
    return SynquiLangGraphHandler(session=session)
